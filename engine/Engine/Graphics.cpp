...
void Graphics::InitD3D(HWND hWnd, float width, float height)
{
	...
	{
		const auto state = CreateDepthStencilState(D3D11_COMPARISON_LESS);
		mDevCon->OMSetDepthStencilState(state, 0);
		state->Release();

		CreateDepthStencil(static_cast<int32_t>(width), static_cast<int32_t>(height), &mDepthStencilTexture, &mDepthStencilView);
	}

	{
		...
		DbgAssert(S_OK == hr, "sampler state failed");
...

	{
		...
		hr = mDev->CreateRasterizerState(&desc, &state);
		DbgAssert(hr == S_OK, "Something went wrong with your GPU!!!!!!!");

		mDevCon->RSSetState(state);
		state->Release();
	}
}
...
void Graphics::SetRenderTarget(ID3D11RenderTargetView* renderTarget, ID3D11DepthStencilView* depthView)
{
	...
	mDevCon->OMSetRenderTargets(1, &mCurrentRenderTarget, depthView);
}
...
ID3D11DepthStencilState* Graphics::CreateDepthStencilState(D3D11_COMPARISON_FUNC inDepthComparisonFunction)
{
	...
	{
		auto hr = mDev->CreateDepthStencilState(&dsDesc, &pDSState);
		DbgAssert(S_OK == hr, "crate state failed");
	}
	return pDSState;
}
	...
	ID3D11DepthStencilView** pDepthView)
{
	...
	{
		auto hr = mDev->CreateTexture2D(&descDepth, NULL, pDepthTexture);
		DbgAssert(S_OK == hr, "depth texture fail");
	}
	{
		auto hr = mDev->CreateDepthStencilView(*pDepthTexture, nullptr, pDepthView);
		DbgAssert(S_OK == hr, "create view failed");
	}
	return true;
}
...
void Graphics::UnbindActiveTexture()
{
	ID3D11ShaderResourceView* clear[TEXTURE_SLOT_TOTAL] = { nullptr };
	mDevCon->PSSetShaderResources(TEXTURE_SLOT_DIFFUSE, TEXTURE_SLOT_TOTAL, clear);
}

void Graphics::UnbindActiveRenderTarget()
{
	ID3D11RenderTargetView* view[1] = { nullptr };
	mDevCon->OMSetRenderTargets(std::size(view), view, nullptr);
}
...
ID3D11BlendState* Graphics::CreateBlendState(bool enable, D3D11_BLEND srcBlend, D3D11_BLEND dstBlend)
{
	...
	rtbd.BlendEnable = enable;
	rtbd.SrcBlend = srcBlend;
	rtbd.DestBlend = dstBlend;
	...
	mDev->CreateBlendState(&blendDesc, &res);
	return res;
}
